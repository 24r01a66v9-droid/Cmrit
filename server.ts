...
async function startServer() {
  ...
  app.post("/api/photos", upload.single("file"), async (req: any, res) => {
    ...
    if (supabase) {
      try {
        ...
        if (error) throw error;
        return res.json({ success: true, id: data[0].id, url: fileUrl });
      } catch (error) {
        console.error("Supabase add photo error:", error);
        // Clean up uploaded file if Supabase fails
    ...
    try {
      ...
      res.json({ success: true, id: result.lastInsertRowid, url: fileUrl });
    } catch (error) {
  ...
  app.delete("/api/photos/:id", async (req, res) => {
    ...
    if (supabase) {
      try {
        ...
        // Delete file from disk if it's stored locally
        if (photoData.url.startsWith("/uploads/")) {
    ...
    try {
      // Fetch photo to get the URL
      const photo = db.prepare("SELECT url FROM photos WHERE id = ?").get(id) as any;
